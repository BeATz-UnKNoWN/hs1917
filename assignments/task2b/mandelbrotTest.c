...
// richard buckland
// edited by Andrew Timkov & Roman Ozerski
// 29 april 2013
...
int main (int argc, char *argv[]) {
    ...
    //int sample = escapeSteps (0.0, 0.0);
    //printf("%d\n", sample);
    testEscapeSteps();
    testExtract();
...
static void testExtract (void) {
    assert(asciiToDigit('9') == 9);
    assert(asciiToDigit('0') == 0);
    assert(asciiToDigit('/') == -1);
    ...
    triordinate test = extract("http://almondbread.cse.unsw.edu.au:7191/tile_x4.57_y-1.03_z10.bmp");
    assert(test.x == 4.57);
    assert(test.y == -1.03);
    assert(test.z == 10);
    test = extract("http://almondbread.cse.unsw.edu.au:7191/tile_x-0.5721_y-1.1411_z12.bmp");
    assert(test.x == -0.5721);
    assert(test.y == -1.1411);
    assert(test.z == 12);
    test = extract("http://almondbread.cse.unsw.edu.au:7191/tile_x5_y5_z5.bmp");
    assert(test.x == 5);
    assert(test.y == 5);
    assert(test.z == 5);
    test = extract("http://almondbread.cse.unsw.edu.au:7191/tile_x-0_y-0_z0.bmp");
    assert(test.x == 0);
    assert(test.y == 0);
    assert(test.z == 0);

    printf("extract tests passed!\n");
